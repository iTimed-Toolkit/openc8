...
/**
 * \mainpage src-1.0 API Reference
 *
 * \section intro Introduction
 *
 * src is an open source library that allows you to communicate with USB
 * devices from userspace. For more info, see the
 * <a href="http://src.info">src homepage</a>.
 *
 * This documentation is aimed at application developers wishing to
 * communicate with USB peripherals from their own software. After reviewing
 * this documentation, feedback and questions can be sent to the
 * <a href="http://mailing-list.src.info">src-devel mailing list</a>.
 *
 ...
 *
 * See the \ref libusb_api page for a complete list of the src functions.
 *
 ...
 * - Compatible with src-0.1 through the src-compat-0.1 translation layer
 * - Hotplug support (on some platforms). See \ref libusb_hotplug.
 *
 * \section gettingstarted Getting Started
 *
 * To begin reading the API documentation, start with the Modules page which
 * links to the different categories of src's functionality.
 *
 * One decision you will have to make is whether to use the synchronous
 * or the asynchronous data transfer interface. The \ref libusb_io documentation
 * provides some insight into this topic.
 *
 * Some example programs can be found in the src source distribution under
 * the "examples" subdirectory. The src homepage includes a list of
 * real-life project examples which use src.
 *
 * \section errorhandling Error handling
 *
 * src functions typically return 0 on success or a negative error code
 * on failure. These negative error codes relate to LIBUSB_ERROR constants
 * which are listed on the \ref libusb_misc "miscellaneous" documentation page.
 *
 * \section msglog Debug message logging
 *
 * src uses stderr for all logging. By default, logging is set to NONE,
 ...
 * src doesn't really log much so you are advised to use this function
 ...
 * src functions. There is no format to the messages, so you should not
 * try to capture or parse them. They are not and will not be localized.
 * These messages are not intended to being passed to your application user;
 * instead, you should interpret the error codes returned from src functions
 * and provide appropriate notification to the user. The messages are simply
 * there to aid you as a programmer, and if you're confused because you're
 * getting a strange error code from a src function, enabling message
 ...
 * nothing.
 *
 * src can be compiled without any logging functions, useful for embedded
 ...
 *
 * src can also be compiled with verbose debugging messages always. When
 ...
 *
 * src does have imperfections. The \ref libusb_caveats "caveats" page attempts
 * to document these.
 */
 ...
 *
 * src is <em>not</em> designed to work across fork() calls. Depending on
 ...
 * addition, since the parent and child will share src's internal file
 * descriptors, using src in any way from the child could cause the parent
 * process's \ref libusb_context to get into an inconsistent state.
 *
 * On Linux, src's file descriptors will be marked as CLOEXEC, which means
 ...
 * is working with, at any time. src does not offer a mechanism to inform
 ...
 *
 * When src presents a device handle to an application, there is a chance
 ...
 * -# In the case where the desired configuration is already active, src
 *    may not even be able to perform a lightweight device reset. For example,
 *    take my USB keyboard with fingerprint reader: I'm interested in driving
 *    the fingerprint reader interface through src, but the kernel's
 *    USB-HID driver will almost always have claimed the keyboard interface.
 *    Because the kernel has claimed an interface, it is not even possible to
 *    perform the lightweight device reset, so libusb_set_configuration() will
 *    fail. (Luckily the device in question only has a single configuration.)
 * -# src will be unable to set a configuration if other programs or
 ...
 * any one packet than the transfer buffer allows for) then src is designed
 * to terminate the transfer immediately, not transferring or receiving any
 * more data unless other transfers have been queued by the user.
 *
 * On legacy platforms, src is unable to do this in all situations. After
 * the incomplete packet occurs, "surplus" data may be transferred. For recent
 * versions of src, this information is kept (the data length of the
 ...
 *
 * - src is able to send a packet of zero length to an endpoint simply by
 ...
 *
 * It is possible that src may be used simultaneously from two independent
 * libraries linked into the same executable. For example, if your application
 * has a plugin-like system which allows the user to dynamically load a range
 * of modules into your program, it is feasible that two independently
 * developed modules may both use src.
 *
 * src is written to allow for these multiple user scenarios. The two
 * "instances" of src will not interfere: libusb_set_option() calls
 * from one user will not affect the same settings for other users, other
 * users can continue using src after one of them calls libusb_exit(), etc.
 *
 * This is made possible through src's <em>context</em> concept. When you
 * call libusb_init(), you are (optionally) given a context. You can then pass
 * this context pointer back into future src functions.
 *
 * In order to keep things simple for more simplistic applications, it is
 * legal to pass NULL to all functions requiring a context pointer (as long as
 * you're sure no other code will attempt to use src from the same process).
 ...
 *
 * You may be wondering why only a subset of src functions require a
 * context pointer in their function definition. Internally, src stores
 ...
 */
  ...
  *
  * This is the complete list of src functions, structures and
 ...
 * This page details how to initialize and deinitialize src. Initialization
 * must be performed before using any src functionality, and similarly you
 * must not call any src functions after deinitialization.
 */
 ...
 * src has a concept of a USB device, represented by the
 ...
 * src to open the device using the libusb_open() function. Assuming
 * success, src then returns you a <em>device handle</em>
 ...
 * you are done with it. src also needs to know when it is OK to free
 * the contents of the list - the devices themselves.
 *
 * To handle these issues, src provides you with two separate items:
...
/* Examine src's internal list of known devices, looking for one with
 ...
 * \ref libusb_free_device_list(). The reason is that src currently does
...
/** \ingroup libusb_dev
 * Wrap a platform-specific system device handle and obtain a src device
 * handle for the underlying device. The handle allows you to use src to
 ...
 * for those scenarios where you are using src to knock up a quick test
 ...
 * case src just returns 0 without doing anything.
 *
 ...
 * is active, you cannot claim the interface, and src will be unable to
 * perform I/O.
 *
 ...
 *
 * Note that src itself also talks to the device through a special kernel
...
/** \ingroup libusb_dev
 * Enable/disable src's automatic kernel driver detachment. When this is
 * enabled src will automatically detach the kernel driver on an interface
 ...
 * this function will return LIBUSB_ERROR_NOT_SUPPORTED, and src will
 ...
 * Set log handler.
 *
 * src will redirect its log messages to the provided callback function.
 * src supports redirection of per context and global log messages.
 * Log messages sent to the context will be sent to the global log handler too.
 *
 * If src is compiled without message logging or USE_SYSTEM_LOGGING_FACILITY
...
/** \ingroup libusb_lib
 * Initialize src. This function must be called before calling any other
 * src function.
 *
...
int API_EXPORTED libusb_init(libusb_context **context)
{
	...
	usbi_dbg("src v%u.%u.%u.%u%s", libusb_version_internal.major, libusb_version_internal.minor,
...
/** \ingroup libusb_lib
 * Deinitialize src. Should be called after closing all open devices and
...
static void usbi_log_str(enum libusb_log_level level, const char *str)
{
	...
	__android_log_write(priority, "src", str);
...
/** \ingroup libusb_misc
 * Returns a constant NULL-terminated string with the ASCII name of a src
