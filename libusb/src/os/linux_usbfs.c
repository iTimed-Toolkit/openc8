...
static int _get_usbfs_fd(struct libusb_device *dev, mode_t mode, int silent)
{
	...
	if (!silent) {
		usbi_err(ctx, "src couldn't open USB device %s: %s",
			 path, strerror(errno));
		if (errno == EACCES && mode == O_RDWR)
			usbi_err(ctx, "src requires write access to USB "
				      "device nodes.");
	}
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (r < 0) {
			 ...
			 *    discarded URBs are reaped, otherwise src will be using
			 *    freed memory.
	...
	struct usbfs_urb *urb)
{
	...
	if (tpriv->reap_action != NORMAL) {
		 ...
		 * (closing any holes), so that src reports the total amount of
	...
	struct usbfs_urb *urb)
{
	...
	usbi_mutex_lock(&itransfer->lock);
	usbi_dbg("handling completion status %d for transfer %p", urb->status, tpriv);
...
static int reap_for_handle(struct libusb_device_handle *handle)
{
	...
	usbi_dbg("urb type=%d status=%d transferred=%d", urb->type, urb->status,
		urb->actual_length);
	usbi_dbg("got usb transfer pointer %p", transfer);
	...
	struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	usbi_dbg("called op_handle_events with ctx=%p, fds=%p, nfds=%i, num_ready=%i", ctx, fds, nfds, num_ready);
