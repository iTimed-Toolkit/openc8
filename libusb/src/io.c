 ...
 *
 * If you're using src in your application, you're probably wanting to
 * perform I/O with devices - you want to perform USB data transfers.
 *
 * src offers two separate interfaces for device I/O. This page aims to
 ...
 *
 * src offers both a synchronous and an asynchronous interface to performing
 ...
 *
 * If you have used the src-0.1 before, this I/O style will seem familar to
 * you. src-0.1 only offered a synchronous interface.
 *
 ...
 *
 * Asynchronous I/O is the most significant new feature in src-1.0.
 * Although it is a more complex interface, it solves all the issues detailed
 * above.
 *
 * Instead of providing which functions that block until the I/O has complete,
 * src's asynchronous interface presents non-blocking functions which
 * begin a transfer and then return immediately. Your application passes a
 * callback function pointer to this non-blocking function, which src will
 ...
 * - In the interest of being a lightweight library, src does not create
 * threads and can only operate when your application is calling into it. Your
 * application must call into src from it's main loop when events are ready
 * to be handled, or you must use some other scheme to allow src to
 * undertake whatever work needs to be done.
 * - src also needs to be called into at certain fixed points in time in
 ...
 *
 * Internally, src's synchronous interface is expressed in terms of function
 ...
 *
 * src and the underlying OS abstract out the packet concept, allowing you
 ...
 *
 * When requesting data on a bulk endpoint, src requires you to supply a
 * buffer and the maximum number of bytes of data that src can put in that
 * buffer. However, the size of the buffer is not communicated to the device -
 * the device is just asked to send any amount of data.
 *
 * There is no problem if the device sends an amount of data that is less than
 * or equal to the buffer size. src reports this condition to you through
 * the \ref libusb_transfer::actual_length "libusb_transfer.actual_length"
 * field.
 *
 * Problems may occur if the device attempts to send more data than can fit in
 * the buffer. src reports LIBUSB_TRANSFER_OVERFLOW for this condition but
 ...
 *
 * This page details src's asynchronous (non-blocking) API for USB device
 ...
 *
 * For the asynchronous I/O, src implements the concept of a generic
 ...
 * -# <b>Submission</b>: ask src to submit the transfer
 ...
 * time, src has populated this structure with results of the transfer:
 ...
 * src that will attempt to perform any event handling. Examples of such
 ...
 * src will communicate this to you in the transfer callback. Do not assume
 ...
 * isochronous transfer. Such limits can be difficult for src to detect,
 ...
 * transfer may return before src has finished using the buffer, and when
 ...
 *   "LIBUSB_TRANSFER_FREE_BUFFER" allows you to ask src to free the transfer
 *   buffer when freeing the transfer.
 * - \ref libusb_transfer_flags::LIBUSB_TRANSFER_FREE_TRANSFER
 *   "LIBUSB_TRANSFER_FREE_TRANSFER" causes src to automatically free the
 *   transfer after the transfer callback returns.
 *
 * \section asyncevent Event handling
 *
 * An asynchronous model requires that src perform work at various
 * points in time - namely processing the results of previously-submitted
 * transfers and invoking the user-supplied callback function.
 *
 * This gives rise to the libusb_handle_events() function which your
 * application must call into when src has work do to. This gives src
 ...
 *    thread.
 * -# Integrate src with your application's main event loop. src
 ...
 * were src' poll API for select / poll integration is not available. So
 ...
 * see the \ref libusb_poll "polling and timing" section for how to integrate src
 * into your application's main event loop.
 *
 * \section eventthread Using an event handling thread
 *
 * Lets begin with stating the obvious: If you're going to use a separate
 * thread for src event handling, your callback functions MUST be
 * threadsafe.
 *
 ...
 * application uses src' \ref libusb_hotplug "hotplug" support or not.
 *
 ...
 *
 * This page documents src's functions for polling events and timing.
 ...
 * asynchronous API documentation. In summary, src does not create internal
 * threads for event processing and hence relies on your application calling
 * into src at certain points in time so that pending events can be handled.
 *
 * Your main loop is probably already calling poll() or select() or a
 * variant on a set of file descriptors for other event sources (e.g. keyboard
 * button presses, mouse movements, network sockets, etc). You then add
 * src's file descriptors to your poll()/select() calls, and when activity
 * is detected on such descriptors you know it is time to call
 * libusb_handle_events().
 *
 * There is one final event handling complication. src supports
 ...
 * fd, on other platforms this requires that src is called into at or after
 * the timeout to handle it. So, in addition to considering src's file
 * descriptors in your main event loop, you must also consider that src
 * sometimes needs to be called into at fixed points in time even when there
 * is no file descriptor activity, see \ref polltime details.
 *
 * In order to know precisely when src needs to be called into, src
 ...
 *
 * If your application revolves solely around src and does not need to
 ...
 * movements, etc. Through exposing a set of file descriptors, src is
 * designed to cleanly integrate into such main loops.
 *
 * In addition to polling file descriptors for the other event sources, you
 * take a set of file descriptors from src and monitor those too. When you
 * detect activity on src's file descriptors, you call
 * libusb_handle_events_timeout() in non-blocking mode.
 *
 * What's more, src may also need to handle events at specific moments in
 ...
 *
 * src provides you with a set of file descriptors to poll and expects you
 * to poll all of them, treating them as a single entity. The meaning of each
 * file descriptor in the set is an internal implementation detail,
 * platform-dependent and may vary from release to release. Don't try and
 * interpret the meaning of the file descriptors, just do as src indicates,
...
while (user has not requested application exit) {
	...
	poll(on src file descriptors plus any other event sources of interest,
		using a timeout no larger than the value src just suggested)
	if (poll() indicated activity on src file descriptors)
		libusb_handle_events_timeout(ctx, &zero_tv);
	if (time has elapsed to or beyond the src timeout)
		libusb_handle_events_timeout(ctx, &zero_tv);
 ...
 *
 * The above complication with having to track time and call into src at
 ...
 * and <em>any future platforms supported by src which may have time-based
 * event requirements</em>). The resultant problems will likely appear as
 * strange bugs in your application.
 *
 * You can use the libusb_pollfds_handle_timeouts() function to do a runtime
 * check to see if it is safe to ignore the time-based event complications.
 * If your application has taken the shortcut of ignoring src's next timeout
 ...
 *
 * The set of file descriptors that src uses as event sources may change
 ...
 *
 * src is a thread-safe library, but extra considerations must be applied
 * to applications which interact with src from multiple threads.
 *
 * The underlying issue that must be addressed is that all src I/O
 ...
 * or select() on src's file descriptors then only one of those threads
 * will be woken up when an event arrives. The others will be completely
 * oblivious that anything has happened.
 *
 * Consider the following pseudo-code, which submits an asynchronous transfer
 * then waits for its completion. This style is one way you could implement a
 * synchronous interface on top of the asynchronous interface (and src
...
void myfunc() {
	...
	while (!completed) {
		poll(src file descriptors, 120*1000);
 ...
 *
 * If this is the only thread that is polling src's file descriptors, there
 ...
 * impact the capabilities of the library, so src offers the scheme
 * documented below to ensure no loss of functionality.
 *
 * Before we go any further, it is worth mentioning that all src-wrapped
 * event handling procedures fully adhere to the scheme documented below.
 * This includes libusb_handle_events() and its variants, and all the
 * synchronous I/O functions - src hides this headache from you.
 *
 ...
 * synchronous API implementation of src.
 *
 ...
 * src-1.0.9 introduces the new libusb_handle_events_timeout_completed()
 ...
 *
 * The problem is when we consider the fact that src exposes file
 * descriptors to allow for you to integrate asynchronous USB I/O into
 * existing main loops, effectively allowing you to do some work behind
 * src's back. If you do take src's file descriptors and pass them to
 ...
 *
 * You must take the events lock before polling src file descriptors,
 ...
 *
 * To solve this, src offers you a mechanism to determine when another
 ...
 * -# libusb_event_handling_ok() checks that src is still happy for your
 *    thread to be performing event handling. Sometimes, src needs to
 ...
 *    "working behind src's back", as is the case here.
 * -# libusb_event_handler_active() determines if someone is currently
 *    holding the events lock
 *
 * You might be wondering why there is no function to wake up all threads
 * blocked on libusb_wait_for_event(). This is because src can do this
 ...
 * questions regarding src's internals. If you're curious, read on, and if
 ...
 *
 * src handles these issues internally, so application developers do not
 * have to stop their event handlers while opening/closing devices. Here's how
 * it works, focusing on the libusb_close() situation first:
 *
 * -# During initialization, src opens an internal pipe, and it adds the read
 *    end of this pipe to the set of file descriptors to be polled.
 * -# During libusb_close(), src writes some dummy data on this event pipe.
 *    This immediately interrupts the event handler. src also records
 ...
 * -# With the events lock held inside libusb_close(), src can safely remove
 ...
 * -# src sends some dummy data on the event pipe, and records that it
 ...
 * applies to applications that take src's file descriptors and integrate
 ...
 * Allocate a src transfer with a specified number of isochronous packet
 ...
 * only one thread is monitoring src event sources at any one time.
 *
 * You only need to use this lock if you are developing an application
 * which calls poll() or select() on src's file descriptors directly.
 * If you stick to src's event handling loop functions (e.g.
 ...
 * monitoring src event sources at any one time.
 *
 * You only need to use this lock if you are developing an application
 * which calls poll() or select() on src's file descriptors directly.
 * If you stick to src's event handling loop functions (e.g.
 ...
 *
 * Sometimes, src needs to temporarily pause all event handlers, and this
...
static int handle_events(struct libusb_context *ctx, struct timeval *tv)
{
	...
	/* there are certain fds that src uses internally, currently:
	 *
	 *   1) event pipe
	 *   2) timerfd
	 *
 ...
 *
 * src determines "pending events" by checking if any timeouts have expired
 ...
 * taken the event lock and are calling poll()/select() directly on src's
 * file descriptors (as opposed to using libusb_handle_events() or similar).
 * You detect events on src's descriptors, so you then call this function
 ...
 * when monitoring src's file descriptors.
 *
 * This function is only useful for applications which retrieve and poll
 * src's file descriptors in their own main loop (\ref libusb_pollmain).
 *
 * Ordinarily, src's event handler needs to be called into at specific
 ...
 *
 * Some platforms supported by src do not come with this baggage - any
 ...
 *
 * \param ctx the context to operate on, or NULL for the default context
 * \returns 0 if you must call into src at times determined by
 * libusb_get_next_timeout(), or 1 if all timeout events are handled internally
 * or through regular activity on the file descriptors.
 * \ref libusb_pollmain "Polling src file descriptors for event handling"
 */
...
/** \ingroup libusb_poll
 * Determine the next internal timeout that src needs to handle. You only
 * need to use this function if you are calling poll() or select() or similar
 * on src's file descriptors yourself - you do not need to use it if you
 * are calling libusb_handle_events() or a variant directly.
 *
 * You should call this function in your main loop in order to determine how
 * long to wait for select() or poll() to return results. src needs to be
 * called into at this timeout, so you should use it as an upper bound on
 * your select() or poll() call.
 *
 * When the timeout has expired, call into libusb_handle_events_timeout()
 * (perhaps in non-blocking mode) so that src can handle the timeout.
 *
 * This function may return 1 (success) and an all-zero timeval. If this is
 * the case, it indicates that src has a timeout that has already expired
 ...
 * clock in which src must be called into in order to process timeout events
 ...
 * that src uses as an event source.
 *
...
#else
	usbi_err(ctx, "external polling of src's internal descriptors "\
