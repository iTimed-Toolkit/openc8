...
GLOBAL_CHAR_ARR(const_rc_lookup,
            0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c);
#else

#include <stdint.h>

static const uint8_t const_sbox[] = {
        0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
        0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
        0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
        0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75,
...
static const uint32_t const_t0[] = {
        0xa56363c6, 0x847c7cf8, 0x997777ee, 0x8d7b7bf6, 0xdf2f2ff, 0xbd6b6bd6, 0xb16f6fde, 0x54c5c591,
        0x50303060, 0x3010102, 0xa96767ce, 0x7d2b2b56, 0x19fefee7, 0x62d7d7b5, 0xe6abab4d, 0x9a7676ec,
        0x45caca8f, 0x9d82821f, 0x40c9c989, 0x877d7dfa, 0x15fafaef, 0xeb5959b2, 0xc947478e, 0xbf0f0fb,
        0xecadad41, 0x67d4d4b3, 0xfda2a25f, 0xeaafaf45, 0xbf9c9c23, 0xf7a4a453, 0x967272e4, 0x5bc0c09b,
...
static const uint32_t const_t1[] = {
        0x6363c6a5, 0x7c7cf884, 0x7777ee99, 0x7b7bf68d, 0xf2f2ff0d, 0x6b6bd6bd, 0x6f6fdeb1, 0xc5c59154,
        0x30306050, 0x1010203, 0x6767cea9, 0x2b2b567d, 0xfefee719, 0xd7d7b562, 0xabab4de6, 0x7676ec9a,
        0xcaca8f45, 0x82821f9d, 0xc9c98940, 0x7d7dfa87, 0xfafaef15, 0x5959b2eb, 0x47478ec9, 0xf0f0fb0b,
        0xadad41ec, 0xd4d4b367, 0xa2a25ffd, 0xafaf45ea, 0x9c9c23bf, 0xa4a453f7, 0x7272e496, 0xc0c09b5b,
...
static const uint32_t const_t2[] = {
        0x63c6a563, 0x7cf8847c, 0x77ee9977, 0x7bf68d7b, 0xf2ff0df2, 0x6bd6bd6b, 0x6fdeb16f, 0xc59154c5,
        0x30605030, 0x1020301, 0x67cea967, 0x2b567d2b, 0xfee719fe, 0xd7b562d7, 0xab4de6ab, 0x76ec9a76,
        0xca8f45ca, 0x821f9d82, 0xc98940c9, 0x7dfa877d, 0xfaef15fa, 0x59b2eb59, 0x478ec947, 0xf0fb0bf0,
        0xad41ecad, 0xd4b367d4, 0xa25ffda2, 0xaf45eaaf, 0x9c23bf9c, 0xa453f7a4, 0x72e49672, 0xc09b5bc0,
...
static const uint32_t const_t3[] = {
        0xc6a56363, 0xf8847c7c, 0xee997777, 0xf68d7b7b, 0xff0df2f2, 0xd6bd6b6b, 0xdeb16f6f, 0x9154c5c5,
        0x60503030, 0x2030101, 0xcea96767, 0x567d2b2b, 0xe719fefe, 0xb562d7d7, 0x4de6abab, 0xec9a7676,
        0x8f45caca, 0x1f9d8282, 0x8940c9c9, 0xfa877d7d, 0xef15fafa, 0xb2eb5959, 0x8ec94747, 0xfb0bf0f0,
        0x41ecadad, 0xb367d4d4, 0x5ffda2a2, 0x45eaafaf, 0x23bf9c9c, 0x53f7a4a4, 0xe4967272, 0x9b5bc0c0,
...
static const uint8_t const_rc_lookup[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c};
#endif
...
void expand_key(unsigned char key[16], unsigned char key_sched[176], int n)
{
...
#ifdef DEV_CRYPTO
    uint8_t *const_sbox = GET_PTR_TO_GLOBAL(const_sbox);
    uint8_t *const_rc_lookup = GET_PTR_TO_GLOBAL(const_rc_lookup);
#endif
    ...
    for(i = 1; i < n; i++)
    {
        ...
        for(j = 0; j < 3; j++)
        {
            ...
            key_sched[key_base + j] = const_sbox[val];
        }

        val = key_sched[prev_key_base + 12];
        key_sched[key_base + 3] = const_sbox[val];

        key_sched[key_base] ^= const_rc_lookup[i - 1];
                               ...
                               unsigned char key_sched[176])
{
...
#ifdef DEV_CRYPTO
    uint8_t *const_sbox;
    uint32_t *const_t0, *const_t1, *const_t2, *const_t3;
    const_sbox = GET_PTR_TO_GLOBAL(const_sbox);
    const_t0 = GET_PTR_TO_GLOBAL(const_t0);
    const_t1 = GET_PTR_TO_GLOBAL(const_t1);
    const_t2 = GET_PTR_TO_GLOBAL(const_t2);
    const_t3 = GET_PTR_TO_GLOBAL(const_t3);
#endif
    ...
    for(i = 1; i < 10; ++i)
    {
        u0 = const_t0[(unsigned char) t0] ^
             const_t1[(unsigned char) (t1 >> 8u)] ^
             const_t2[(unsigned char) (t2 >> 16u)] ^
             const_t3[(unsigned char) (t3 >> 24u)];

        u1 = const_t0[(unsigned char) t1] ^
             const_t1[(unsigned char) (t2 >> 8u)] ^
             const_t2[(unsigned char) (t3 >> 16u)] ^
             const_t3[(unsigned char) (t0 >> 24u)];

        u2 = const_t0[(unsigned char) t2] ^
             const_t1[(unsigned char) (t3 >> 8u)] ^
             const_t2[(unsigned char) (t0 >> 16u)] ^
             const_t3[(unsigned char) (t1 >> 24u)];

        u3 = const_t0[(unsigned char) t3] ^
             const_t1[(unsigned char) (t0 >> 8u)] ^
             const_t2[(unsigned char) (t1 >> 16u)] ^
             const_t3[(unsigned char) (t2 >> 24u)];
    ...
    msg_int[0] = ((unsigned int) const_sbox[(unsigned char) (t0 >> 0u)] << 0u |
                  (unsigned int) const_sbox[(unsigned char) (t1 >> 8u)] << 8u |
                  (unsigned int) const_sbox[(unsigned char) (t2 >> 16u)] << 16u |
                  (unsigned int) const_sbox[(unsigned char) (t3 >> 24u)] << 24u) ^
                 key_sched_int[4 * i];

    msg_int[1] = ((unsigned int) const_sbox[(unsigned char) (t1 >> 0u)] << 0u |
                  (unsigned int) const_sbox[(unsigned char) (t2 >> 8u)] << 8u |
                  (unsigned int) const_sbox[(unsigned char) (t3 >> 16u)] << 16u |
                  (unsigned int) const_sbox[(unsigned char) (t0 >> 24u)] << 24u) ^
                 key_sched_int[4 * i + 1];

    msg_int[2] = ((unsigned int) const_sbox[(unsigned char) (t2 >> 0u)] << 0u |
                  (unsigned int) const_sbox[(unsigned char) (t3 >> 8u)] << 8u |
                  (unsigned int) const_sbox[(unsigned char) (t0 >> 16u)] << 16u |
                  (unsigned int) const_sbox[(unsigned char) (t1 >> 24u)] << 24u) ^
                 key_sched_int[4 * i + 2];

    msg_int[3] = ((unsigned int) const_sbox[(unsigned char) (t3 >> 0u)] << 0u |
                  (unsigned int) const_sbox[(unsigned char) (t0 >> 8u)] << 8u |
                  (unsigned int) const_sbox[(unsigned char) (t1 >> 16u)] << 16u |
                  (unsigned int) const_sbox[(unsigned char) (t2 >> 24u)] << 24u) ^
                 key_sched_int[4 * i + 3];
}